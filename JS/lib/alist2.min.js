...
const VERSION = "alist v2/v3 202404210825";
...
const http = function (url, options = {}, noAuth = false, timeout = request_timeout) {
  if (options.method === "POST" && options.data) {
    ...
    if (!noAuth) {
      options.headers = Object.assign({
        'Authorization': 'alist-09ceb38a-f143-47f7-b255-c3eec819cd7bOafUq_TDeD88KeT4MBVJNWgPvR1P3nukZnB1N92hVi5UIlzDU38Z8n92dK5OfzzZ'
      }, options.headers);
    }
  }
...
function getAllVods(para, drivename, pg = "0", filter = false, extend = {}) {
  ...
  lists.forEach(it => {
    ...
    let emojiRegex = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g;
    let emojis = parts[0].match(emojiRegex);
    let emoji = (emojis && emojis.length > 0) ? emojis[0] : "";
    let folderIcon = isMedia(parts[0]) ? "" : "\u{1F4C2}";
    if (isPikpak) {
      emoji = emoji.replace("\u{1F578}", "");
      emoji = "\u{1F578}" + emoji;
    }
    let icons = emoji + folderIcon;
